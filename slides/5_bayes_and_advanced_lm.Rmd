...
pred_out<-as_tibble(posterior_predict(m1_bayes, newdata = fake_data)) %>% 
  pivot_longer(cols = everything(), names_to = "index", values_to = "mpg_hat") %>% 
  mutate(mpg_hat = as.numeric(mpg_hat))
...
m2_bayes_noscale<-stan_glm(mpg ~ wt + hp,
                   data = mtcars)
...
```{r}
m4_bayes<-stan_glm(mpg ~ scale(wt) + scale(hp) + scale(cyl)+
                     scale(wt)*scale(hp),
                   data = mtcars)
...
### predict over each observed value of mpg
### and make it tidy, because tidy is nice
m3_preds<-as_tibble(posterior_predict(m3_bayes)) %>% 
  pivot_longer(cols = everything())
