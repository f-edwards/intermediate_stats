...
This model, with no included parameters for group differences, is called a *complete pooling* model. Data from all groups is used to estimate a single global parameter.
...
\[\alpha_j \propto \bar{L_j} \times p(\alpha_j)\]
...
Our model *regularizes* (or shrinks) intercept estimates by gently nudging them toward the prior. 
...
ggplot(plot_dat_all, 
  ...
  geom_point() +
  facet_wrap(~model) + 
  labs(y = "Absolute difference from observed")
```
