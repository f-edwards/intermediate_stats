...
ggplot(sim_mcar, aes(x = yr, y = obs)) + geom_point() + geom_smooth(method = "lm")
```
...
```{r, message = FALSE}
imps<-mice(sim_mcar, m = 100, method = "norm")
sims_imp<-complete(imps, include = TRUE, action = "long") 


temp<-sims_imp %>% filter(.id==6)
```
