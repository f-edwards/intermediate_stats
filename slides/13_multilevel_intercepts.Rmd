...
- Repeated observations of counties over time (longitudinal or time series data)
- Observations of many counties from the same time-period (cross-sectional data)
- Counties are nested within larger geographies (states, regions)
...
Let's say that we think that states may have different characteristics that lead states to have violent crime arrest rates. 

- Theoretically, this could include things like law, economics, politics, history, or population features.
...
*Complete pooling* shares information across states, but makes no distinctions across states

\[E(y_c) = \beta_0 + \beta_1 x_{c}\]

*No pooling* does not share information across states for computing an intercept. Each states gets its own.
...
- Fixed effects adjust for differences in average violent arrests rates *within-counties across states*
...
- Let's assume that violent crime rates in counties *within* states are similar to each other
- But let's also assume that violent crime rates in counties *across*  similar units states.
...
- Under a partial-pooling model, we estimate an intercept for every state based based on a model that *assumes that state intercepts are generated from an underlying probability distribution*
...
We use the `lmer` function from the `lme4` package for multilevel modeling in R
...
The `(1|Variable)` notation is used to specify which categorical measures we'd like to estimate our intercepts for. 
...
We have effectively divided our residual into a *state* component ($\alpha$), and a *county* component $\varepsilon$
...
For our model, we can compute ICC as $\frac{\sigma^2_\alpha}{\sigma^2_\alpha + \sigma^2} = \frac{2.4}{2.4 + 3.8} = 0.39$

This tells us that 39 percent of the differences in violent crime rates across counties are explained by what state that they are in! (after accounting for poverty in the county)
