...
```{r}
p2<-inv_logit(-4.37 + 1.06 *2)
p2.5<-inv_logit(-4.37 + 1.06 *2.5)

diff2<-p2.5 - p2

p3<-inv_logit(-4.37 + 1.06 *3)
p3.5<-inv_logit(-4.37 + 1.06 *3.5)
...
```{r}
m1<-stan_glm(admit ~ gpa, 
             data = admissions,
             family = "binomial")

### FOR A BAYESIAN MODEL
m2<-stan_glm(admit ~ gpa + gre, 
             data = admissions,
             family = "binomial")


new_dat<-tibble(gpa = 3.39,
                gre = seq(220, 800, by = 10))

ppd<-posterior_linpred(m2, newdata = new_dat)

ppd<-as_tibble(ppd) %>% 
  pivot_longer(cols = everything()) %>% 
  mutate(name = as.numeric(name)) %>% 
  group_by(name) %>% 
  summarise(p_med = inv_logit(median(value)),
            p_lwr = inv_logit(quantile(value, 0.05)),
            p_upr = inv_logit(quantile(value, 0.95)))

new_dat<-new_dat %>% 
  bind_cols(ppd)

ggplot(new_dat, 
       aes(x= gre, y = p_med, ymin = p_lwr, ymax = p_upr)) +
  geom_line()+ 
  geom_ribbon(alpha= 0.5)

#### FOR A FREQUENTIST MODEL
new_dat<-tibble(gpa = 3.39,
                gre = seq(220, 800, by = 10))
...
new_dat<-new_dat %>% 
  mutate(yhat = predict(m2, new_dat, type = "response"))

ggplot(new_dat, aes(x = gre, y = yhat)) + 
  geom_line() + 
  ylim(0, 0.6)
```
...
#### FOR A FREQUENTIST MODEL
new_dat<-tibble(gpa = 3.39,
                gre = rep(seq(220, 800, length.out = 100), 4),
                rank = rep(seq(1, 4, by = 1), each = 100))

new_dat<-new_dat %>% 
  mutate(yhat = predict(m3, new_dat))
...
### FAVOR THE MODEL WITH THE LOWER SCORE HERE
AIC(m3)
AIC(m2)


m3<-stan_glm(admit ~ gpa + gre + factor(rank),
        data = admissions)

m4<-glm(admit ~ gpa * gre,
        data = admissions)
...
### USE LEAVE ONE OUT CROSS VALIDATION FOR BAYES
loo_m2<-loo(m2)
loo_m3<-loo(m3)

loo_compare(loo_m2, loo_m3)

m4<-stan_glm(admit ~ gpa * gre,
        data = admissions)

loo_m4<-loo(m4)

loo_compare(loo_m2, loo_m3, loo_m4)
```
